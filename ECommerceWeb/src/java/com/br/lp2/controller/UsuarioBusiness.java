...
public class UsuarioBusiness extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet UsuarioBusiness</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet UsuarioBusiness at " + request.getContextPath() + "</h1>");
            ...
            if (command.endsWith("login")) {
                ...
                if (usuario == null) {
                    request.getSession().setAttribute("erromsg", "Usuário não encontrado!");
                    response.sendRedirect("erro.jsp");
                } else if (password.equals(usuario.getPassword())) {
                    request.getSession().setAttribute("usuario", usuario);
                    response.sendRedirect("home.jsp");
                } else {
                    request.getSession().setAttribute("erromsg", "Senha incorreta!");
                    response.sendRedirect("erro.jsp");
                }
